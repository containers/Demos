...
Build bootable OCI Image"
    ...
    exec_color "podman build --security-opt label=disable -v $HOME/.local/share/containers:/tmp/containers -v $XDG_RUNTIME_DIR/containers/auth.json:/run/containers/0/auth.json --no-cache --cap-add SYS_ADMIN --build-arg TAG=1.0 --build-arg \"SSHPUBKEY=$(cat $HOME/.ssh/id_rsa.pub)\"  --build-arg=\"REGISTRY=$REGISTRY\" --arch=${ARCH} $FROM--manifest ${IMAGE} $APP/"
}
...
Clone AI and explore what is available for use with the Podman Desktop AI Studio"
    ...
    exec_color "podman build -t ${REGISTRY}/chatbot:1.0 -f ai-lab-recipes/recipes/natural_language_processing/chatbot/builds/Containerfile ai-lab-recipes/recipes/natural_language_processing/chatbot"

    exec_color "podman push ${REGISTRY}/chatbot:1.0"
}
